"""
Render "74K4" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
Technique adapted from AVIVASHISHTA29/AVIVASHISHTA29.
"""
import argparse
import html
import math
...
# ---- geometry / grid ------------------------------------------------------
# four glyphs need more columns than the reference's three, or the digits'
# counters collapse into blobs at this cell size.
COLS = int(os.environ.get("WORDMARK_COLS", 72))
ROWS = 0               # derived from the art -- see fit()
# blank rows above and below the art, tuned (with the about-footer) so the
# panel lands at the same displayed height as the portrait beside it.
...
# the letters the same size and lets the extrusion carry the 3D on its own.
...
# printed under the flipbook, terminal-style; fills the panel to the same
# displayed height as the portrait with something worth reading.
FOOTER_CMD = "taka@github: ~$ cat about.yml"
FOOTER_LINES = [
    ("name", "Taka"),
...
def emit(frames, mode, out, dur, reveal):
    ...
    footer_h = FOOTER_LINE_H * (len(FOOTER_LINES) + 2) + 10
    canvas_h = TITLEBAR_H + art_h + footer_h + PAD
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">taka@github: ~$ ./wordmark.sh --3d</text>')
